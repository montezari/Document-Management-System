...
if(isset($_REQUEST["logout"]) && $_REQUEST["logout"] == 1){
  if($_REQUEST["confirm"] == 1){
    session_unset(md5("WWW_DUTADOC_WEB_SESID"));
	unset($_SESSION["cLoggedName"]);
	unset($_SESSION["Logged"]);
    echo "<script>window.location = 'index.php';</script>";
  }else{
    echo "<script>if(confirm('Anda yakin ingin logout?')){window.location = 'index.php?m=admin.auth&logout=1&confirm=1';}else{history.back(-1);}</script>";
  }  
}else{
  ...
  if($rs->RecordCount() > 0){  
    ...
    $conn->Execute($sql);
    $_SESSION["Logged"]	= $rs->fields["cIdUser"];
